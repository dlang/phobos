...
Additionally, archives are checked for malicious data and rejected if detected.
This includes
$(UL
$(LI $(LINK2 https://news.ycombinator.com/item?id=20352439, zip bombs) which
     generate gigantic amounts of unpacked data,)
$(LI zip archives that contain overlapping records, and)
$(LI chameleon zip archives which generate different unpacked data, depending
     on the implementation of the unpack algorithm.)
)

Further checks for potentially harmful data can be applied to an archive by
calling `validate()`. These checks test for typical traces of such data,
but in some cases this may produce false positives. These checks include
$(UL
$(LI directory traversal, directly or by symlinks, and)
$(LI filenames containing suspicious symbols.)
)
See $(LREF .ArchiveMember.validate) for more details.
...
void main(string[] args)
{
    ...
    // check zip file for potentially harmful data
    zip.validate();
...
final class ArchiveMember
{
    ...
    /**
     * Checks the contents of all members for typical traces of potentially
     * harmful data.
     *
     * These checks are applied:
     *
     * $(UL
     * $(LI filenames with more than 255 characters,)
     * $(LI paths and filenames containing unprintable characters
     *      (0x00-0x1F, 0x7F) or backslashes,)
     * $(LI paths that contain directory traversal patterns,)
     * $(LI directorys which pretend to be files, and)
     * $(LI symlinks that contain directory traversal patterns
     *      (symlinks will be expanded for that test).)
     * )
     *
     * Throws: ZipException when potentially harmful data is detected.
     */
    void validate()
    {
        ...
        // Longer names are probably harmful data and we reject them therefore.
        ...
        // reject directory traversal (relative or absolute)
        enforce!ZipException(splitter(name, "/").all!(a => a != "..")
                             && (name.length == 0 || name[0] != '/')
                             && (name.length < 2 || !isAlpha(name[0]) || name[1] != ':'),
                             "filename with directory traversal found");

        // directories must have size 0
        if (isDirectory())
            enforce!ZipException(_compressedSize == 0 && _expandedSize == 0,
                                 "contains directory which pretends to be a file");

        // reject directory traversal with symlink (relative or absolute)
        if (isSymlink())
        {
            auto link = cast(string) expand();
...
private:
    bool isDirectory()
    {
        // we assume, it is a directory, if the filename ends with a slash
        if (name.length > 0 && name[$-1] == '/') return true;

        // DOS directory bit (is also used by other OSs)
        if (_externalAttributes & 0x10) return true;

        // version is UNIX and dir attribute is set
        if ((_madeVersion >> 8) == 3 && (_externalAttributes & 0xF0000000) == 0x40000000) return true;
    ...
    bool isSymlink()
    {
        // version is UNIX and symlink attribute is set
        return (_madeVersion >> 8) == 3 && (_externalAttributes & 0xF0000000) == 0xA0000000;
    }
    ...
    ubyte[] expand()
    {
        switch (compressionMethod)
        {
            ...
            case CompressionMethod.deflate:
                ...
                return cast(ubyte[]) uncompress(cast(void[]) compressedData, expandedSize, -15);
...
public:
     ...
     *
     * Throws: ZipException when the archive was invalid or when malicious data
     * was detected.
     */
    this(void[] buffer)
    {
    ...
    ubyte[] expand(ArchiveMember de)
    {
        ...
        de._expandedData = de.expand();
        return de._expandedData;
    }
    ...
    void validate()
    {
        foreach (de; directory)
            de.validate();
    }

    // issue #20290: Rejects file names exceeding 255 bytes
    @system unittest
    {
        import std.exception : assertThrown, assertNotThrown;
        ...
        auto file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x01\x1c\x00"~
            repeat('\x66').take(256).array~"\x66\x69"~
            "\x6c\x65\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x01\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00"~repeat('\x66').take(256).array~
            "\x66\x69\x6c\x65\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x01\x00\x00\x43\x01\x00"~
            "\x00\x00\x00";

        auto za = new ZipArchive(cast(void[]) file);
        assertThrown!ZipException(za.validate());

        // path + too long filename
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x01\x1c\x00\x66\x69"~
            "\x6c\x2f"~repeat('\x66').take(256).array~
            "\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x01\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x69\x6c\x2f"~repeat('\x66').take(256).array~
            "\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x01\x00\x00\x43\x01\x00"~
            "\x00\x00\x00";

        za = new ZipArchive(cast(void[]) file);
        assertThrown!ZipException(za.validate());

        // path + short enough filename
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x01\x1c\x00\x66\x2f"~
            "\x6c\x66\x2f"~repeat('\x66').take(255).array~
            "\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x01\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x2f\x6c\x66\x2f"~repeat('\x66').take(255).array~
            "\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x01\x00\x00\x43\x01\x00"~
            "\x00\x00\x00";
        ...
        za = new ZipArchive(cast(void[]) file);
        assertNotThrown!ZipException(za.validate());
    }

    // issue #20290: Rejects file names containing invalid characters
    @system unittest
    {
        import std.exception : assertThrown, assertNotThrown;
        ...
        auto file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x00\x1c\x00\x66\x00"~
            "\x6c\x65\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x00\x6c\x65\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x43\x00\x00"~
            "\x00\x00\x00";
        ...
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x00\x1c\x00\x66\x5c"~
            "\x6c\x65\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x5c\x6c\x65\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x43\x00\x00"~
            "\x00\x00\x00";
        ...
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x00\x1c\x00\x66\x1b"~
            "\x6c\x65\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x1b\x6c\x65\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x43\x00\x00"~
            "\x00\x00\x00";
    ...
    // issue #20290: Rejects directory traversal
    @system unittest
    {
        import std.exception : assertThrown, assertNotThrown;
        ...
        auto file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x00\x1c\x00\x2f\x69"~
            "\x6c\x2f\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x2f\x69\x6c\x2f\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x43\x00\x00"~
            "\x00\x00\x00";
        ...
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04\x00\x1c\x00\x66\x3a"~
            "\x6c\x65\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x04"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x3a\x6c\x65\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x43\x00\x00"~
            "\x00\x00\x00";
        ...
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06\x00\x1c\x00\x66\x2f"~
            "\x2e\x2e\x2f\x66\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x2f\x2e\x2e\x2f\x66\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4c\x00\x00\x00\x45\x00\x00"~
            "\x00\x00\x00";
        ...
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06\x00\x1c\x00\x66\x2f"~
            "\x6c\x65\x2f\x66\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x2f\x6c\x65\x2f\x66\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4c\x00\x00\x00\x45\x00\x00"~
            "\x00\x00\x00";
        ...
        // relative path, containing .. but no directory traversal
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x8f\x72\x4a\x4f\x86\xa6"~
            "\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06\x00\x1c\x00\x66\x66"~
            "\x2e\x2e\x2f\x66\x55\x54\x09\x00\x03\x0d\x22\x9f\x5d\x12\x22\x9f\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x68\x65"~
            "\x6c\x6c\x6f\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x8f"~
            "\x72\x4a\x4f\x86\xa6\x10\x36\x05\x00\x00\x00\x05\x00\x00\x00\x06"~
            "\x00\x18\x00\x00\x00\x00\x00\x01\x00\x00\x00\xb0\x81\x00\x00\x00"~
            "\x00\x66\x66\x2e\x2e\x2f\x66\x55\x54\x05\x00\x03\x0d\x22\x9f\x5d\x75\x78"~
            "\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05"~
            "\x06\x00\x00\x00\x00\x01\x00\x01\x00\x4c\x00\x00\x00\x45\x00\x00"~
            "\x00\x00\x00";
    ...
    // issue #20290: Rejects directory traversal with symlinks
    @system unittest
    {
        import std.exception : assertThrown, assertNotThrown;

        // deflated symlink path traversal (relative)
        auto file =
            "\x50\x4b\x03\x04\x14\x00\x00\x00\x08\x00\xe8\x6a\x75\x4f\x19\x40"~
            "\x75\x98\x10\x00\x00\x00\x12\x00\x00\x00\x04\x00\x1c\x00\x6c\x69"~
            "\x6e\x6b\x55\x54\x09\x00\x03\xb4\x81\xd6\x5d\xb8\x81\xd6\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\xd3\xd3"~
            "\xd3\xd7\x03\xa3\x92\xdc\x02\xfd\xfc\xfc\xfc\x82\x62\x00\x50\x4b"~
            "\x01\x02\x1e\x03\x14\x00\x00\x00\x08\x00\xe8\x6a\x75\x4f\x19\x40"~
            "\x75\x98\x10\x00\x00\x00\x12\x00\x00\x00\x04\x00\x18\x00\x00\x00"~
            "\x00\x00\x01\x00\x00\x00\xff\xa1\x00\x00\x00\x00\x6c\x69\x6e\x6b"~
            "\x55\x54\x05\x00\x03\xb4\x81\xd6\x5d\x75\x78\x0b\x00\x01\x04\xf0"~
            "\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05\x06\x00\x00\x00\x00"~
            "\x01\x00\x01\x00\x4a\x00\x00\x00\x4e\x00\x00\x00\x00\x00";

        auto za = new ZipArchive(cast(void[]) file);
        assertThrown!ZipException(za.validate());

        // symlink path traversal absolute (POSIX)
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x0d\x6c\x75\x4f\x2e\xc4"~
            "\xbb\x0a\x04\x00\x00\x00\x04\x00\x00\x00\x04\x00\x1c\x00\x6c\x69"~
            "\x6e\x6b\x55\x54\x09\x00\x03\xda\x83\xd6\x5d\xdc\x83\xd6\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x2f\x74"~
            "\x6d\x70\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x0d\x6c"~
            "\x75\x4f\x2e\xc4\xbb\x0a\x04\x00\x00\x00\x04\x00\x00\x00\x04\x00"~
            "\x18\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xa1\x00\x00\x00\x00"~
            "\x6c\x69\x6e\x6b\x55\x54\x05\x00\x03\xda\x83\xd6\x5d\x75\x78\x0b"~
            "\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05\x06"~
            "\x00\x00\x00\x00\x01\x00\x01\x00\x4a\x00\x00\x00\x42\x00\x00\x00"~
            "\x00\x00";

        za = new ZipArchive(cast(void[]) file);
        assertThrown!ZipException(za.validate());

        // symlink path traversal absolute (DOS)
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\x0d\x6c\x75\x4f\x2e\xc4"~
            "\xbb\x0a\x04\x00\x00\x00\x04\x00\x00\x00\x04\x00\x1c\x00\x6c\x69"~
            "\x6e\x6b\x55\x54\x09\x00\x03\xda\x83\xd6\x5d\xdc\x83\xd6\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x61\x3a"~
            "\x6d\x70\x50\x4b\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\x0d\x6c"~
            "\x75\x4f\x2e\xc4\xbb\x0a\x04\x00\x00\x00\x04\x00\x00\x00\x04\x00"~
            "\x18\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xa1\x00\x00\x00\x00"~
            "\x6c\x69\x6e\x6b\x55\x54\x05\x00\x03\xda\x83\xd6\x5d\x75\x78\x0b"~
    ...
    // issue #20290: Rejects directories with size > 0
    @system unittest
    {
        import std.exception : assertThrown;

        // detect directory by trailing /
        auto file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x1c\x00\x64\x69"~
            "\x72\x2f\x55\x54\x09\x00\x03\xff\x84\xd6\x5d\xff\x84\xd6\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b"~
            "\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x18\x00\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x64\x69\x72\x2f"~
            "\x55\x54\x05\x00\x03\xff\x84\xd6\x5d\x75\x78\x0b\x00\x01\x04\xf0"~
            "\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05\x06\x00\x00\x00\x00"~
            "\x01\x00\x01\x00\x4a\x00\x00\x00\x3e\x00\x00\x00\x00\x00";

        auto za = new ZipArchive(cast(void[]) file);
        assertThrown!ZipException(za.validate());

        // detect directory by DOS directory bit
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x1c\x00\x64\x69"~
            "\x72\x72\x55\x54\x09\x00\x03\xff\x84\xd6\x5d\xff\x84\xd6\x5d\x75"~
            "\x78\x0b\x00\x01\x04\xf0\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b"~
            "\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x18\x00\x00\x00"~
            "\x00\x00\x00\x00\x10\x00\x00\x00\x00\x00\x00\x00\x64\x69\x72\x72"~
            "\x55\x54\x05\x00\x03\xff\x84\xd6\x5d\x75\x78\x0b\x00\x01\x04\xf0"~
            "\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05\x06\x00\x00\x00\x00"~
            "\x01\x00\x01\x00\x4a\x00\x00\x00\x3e\x00\x00\x00\x00\x00";
        ...
        // detect directory by UNIX directory bit
        file =
            "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x1c\x00\x64\x69"~
            "\x72\x72\x55\x54\x09\x00\x03\xff\x84\xd6\x5d\xff\x84\xd6\x5d\x75"~
            ...
            "\x01\x02\x1e\x03\x0a\x00\x00\x00\x00\x00\xaa\x6c\x75\x4f\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x04\x00\x18\x00\x00\x00"~
            "\x00\x00\x00\x00\x00\x00\xf8\x41\x00\x00\x00\x00\x64\x69\x72\x72"~
            "\x55\x54\x05\x00\x03\xff\x84\xd6\x5d\x75\x78\x0b\x00\x01\x04\xf0"~
            "\x03\x00\x00\x04\xf0\x03\x00\x00\x50\x4b\x05\x06\x00\x00\x00\x00"~
            "\x01\x00\x01\x00\x4a\x00\x00\x00\x3e\x00\x00\x00\x00\x00";
