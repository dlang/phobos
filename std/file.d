...
void[] read(R)(R name, size_t upTo = size_t.max, bool appendNullTerminator = false)
if (isSomeFiniteCharInputRange!R && !isConvertibleToString!R)
{
	static if (isNarrowString!R && is(immutable ElementEncodingType!R == immutable char))
		return readImpl(name, name.tempCString!FSChar(), upTo, appendNullTerminator);
	else
		return readImpl(null, name.tempCString!FSChar(), upTo, appendNullTerminator);
}
...
version (Posix) private void[] readImpl(scope const(char)[] name, scope const(FSChar)* namez,
							   size_t upTo = size_t.max, bool appendNullTerminator = false) @trusted
{
	...
	// Calculate initial allocation size, adding 1 for potential null terminator
	...
	// Adjust read limit if we need space for null terminator
	immutable readLimit = appendNullTerminator ? min(upTo, upTo == size_t.max ? size_t.max : upTo - 1) : upTo;

	for (;;)
	{
		immutable actual = core.sys.posix.unistd.read(fd, result.ptr + size.get,
				(min(result.length - (appendNullTerminator ? 1 : 0), readLimit) - size).get);
		cenforce(actual != -1, name, namez);
		if (actual == 0) break;
		size += actual;
		if (size >= readLimit) break;
		if (size < result.length - (appendNullTerminator ? 1 : 0)) continue;
		immutable newAlloc = size + sizeIncrement + (appendNullTerminator ? 1 : 0);
