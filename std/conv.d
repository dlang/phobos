...
/+
emplaceRef is a package function for phobos internal use.
It's implementation has been moved to `core.internal.lifetime`.
+/
package import core.internal.lifetime : emplaceRef;

version (StdDdoc)
{
    /**
    Given a raw memory area `chunk`, constructs an object of type `T` in-place.
    The constructor is passed the arguments `args`, if any.

    For the complete documentation please consult the
    $(LINK2 https://dlang.org/phobos-prerelease/core/lifetime.html, lifetime) module.
...
/+
These `emplace` tests can't be performed in druntime, so, for now, we'll keep them in Phobos.
+/
@safe unittest //@@@9559@@@
{
