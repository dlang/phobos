...
a value of _container type, `n$(SUBSCRIPT x)` represents the effective length of
...
$(TR
    $(TDNW `c ~ x`)
    $(TDNW `n$(SUBSCRIPT c) + n$(SUBSCRIPT x)`)
...
$(TR
    $(TDNW `c.insert(x)`)
    $(TDNW `n$(SUBSCRIPT x) * log n$(SUBSCRIPT c)`)
...
$(TR
    $(TDNW `c.remove(r)`)
    $(TDNW `n$(SUBSCRIPT r) * log n$(SUBSCRIPT c)`)
...
Complexity: $(BIGOH log(e - capacity)) if `e > capacity`, otherwise
$(BIGOH 1).
 */
...
howMany) instructs removal of that many elements. If `howMany > n`,
