    ...
    */
    Lhs hookOpCast(Lhs, Rhs)(Rhs rhs)
    {
        ...
        else
        {
            ...
            // bounds of Lhs
            static if (ProperCompare.hookOpCmp!(Rhs, Lhs)(Rhs.min, Lhs.min) < 0)
            {
                // Example: hookOpCast!short(int(42)), hookOpCast!uint(int(42))
                if (ProperCompare.hookOpCmp!(Rhs, Lhs)(rhs, Lhs.min) < 0)
                    return defaultValue!Lhs;
            }
            static if (ProperCompare.hookOpCmp!(Rhs, Lhs)(Rhs.max, Lhs.max) > 0)
            {
                // Example: hookOpCast!int(uint(42))
                if (ProperCompare.hookOpCmp!(Rhs, Lhs)(rhs, Lhs.max) > 0)
                    return defaultValue!Lhs;
            }
            return cast(Lhs) rhs;
        }
