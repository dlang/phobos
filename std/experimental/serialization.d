...
/**
 Indocates the method of storage used
 */
...
/**
 Converts $(D T from) into an array of bytes
 */ 
...
/**
 Converts $(D ubyte[] bytes) into an $(D T)
 */ 
...
ubyte[] serialize(T)(ref in T source) if (isSerializable!T)
{
	...
	void serializeStructure(U)(ref in U input) if (isSerializableStructure!U)
	{
		...
		else
		{
			...
			{
				static if (isSerializableStructure!(typeof(member)))
				{
					serializeStructure(member);
				}
				else static if (isSerializableData!(typeof(member)))
				{
...
T deserialize(T)(in ubyte[] input)
{
	...
	void deserializeStructure(U)(ref U output)
	{
		...
		else
		{
			...
			foreach(ref member;output.tupleof)
			{
				static if (isSerializableStructure!(typeof(member)))
				{
					deserializeStructure(member);
				}
				else static if (isSerializableData!(typeof(member)))
				{
					deserializeData(member);
				}
			}
		}
	}
