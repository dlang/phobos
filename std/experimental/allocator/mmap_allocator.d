...
Allocator (currently defined only for Posix and Windows) using `$(LUCKY mmap)`
...
`mmap(null, s, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)`,
and each call to `deallocate(b)` issues `munmap(b.ptr, b.length)`.
