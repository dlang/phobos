...
/**
 * Computes SHA1 digests of arbitrary data, using optimized algorithm with SSE3 instructions.
 *
 ...
 * As you can see, the last formula requires value W[i] computed with the first formula.
 ...
 *
 * The 32bit and 64bit implementation are basically identical. The main difference is that 32bit
 * mode there are only 8 XMM registers in 32bit mode. Therefore, space on the stack is needed to
 ...
 */
module std.internal.digest.sha1_SSSE3;
...
version(USE_SSSE3)
{
    ...
    private string constant(uint i)
    {
        return "[constants + 16*"~to!string(i/20)~"]";
    }

    /** Returns the XMM register number used in round i */
    private uint regno(uint i)
    {
        return (i/4)&7;
    }

    /** Returns reference to storage of vector W[i..i+4]. */
    private string WiV(uint i)
    {
        return "["~SP~" + WI_PTR + "~to!string((i/4)&7)~"*16]";
    }

    /** Returns reference to storage of vector (W + K)[i..i+4]. */
    private string WiKiV(uint i)
    {
        return "["~SP~" + WI_PLUS_KI_PTR + "~to!string((i/4)&3)~"*16]";
    }

    /** Returns reference to storage of value W[i] + K[i]. */
    private string WiKi(uint i)
    {
        return "["~SP~" + WI_PLUS_KI_PTR + 4*"~to!string(i&15)~"]";
    }

    /**
     * Chooses the instruction sequence based on the 32bit or 64bit model.
     */
    private string[] swt3264(string[] insn32, string[] insn64)
    {
        version(_32Bit)
        {
            return insn32;
     ...
     */
    private string wrap(string[] insn)
    {
        string s = "asm {";
        foreach (t; insn) s ~= (t ~ "; \n");
        s ~= "}";
     ...
     */
    private string[] weave(string[] seq1, string[] seq2, uint dist = 1)
    {
        string[] res = [];
        auto i1 = 0, i2 = 0;
    ...
    private string[] F(int i, string b, string c, string d)
    {
        string[] insn;
    ...
    private string[] xsetup(int i)
    {
        if (i == 0)
        {
            // Yet another bug: data is not aligned
            return swt3264(["movdqa "~X_SHUFFLECTL~",[bswap_shufb_ctl]",
                             "movdqa "~X_CONSTANT~","~constant(i)],
                            ["movdqu "~X_SHUFFLECTL~",[bswap_shufb_ctl]",
                             "movdqu "~X_CONSTANT~","~constant(i)]);
        }
        version(_64Bit)
        {
            if (i%20 == 0)
            {
                // Yet another bug: data is not aligned (e.g, issue 5355; 64bit only)
                //return ["movdqa "~X_CONSTANT~","~constant(i),
                //       ];
                return ["movdqu "~X_CONSTANT~","~constant(i),
                       ];
            }
        }
        return [];
     ...
     * buffer.
     */
    private string[] precalc_00_15(int i)
    {
        int regno = regno(i);

        string W = "XMM" ~ to!string(regno);
        version(_32Bit)
        {
            string W_TMP = "XMM" ~ to!string(regno+2);
        }
        version(_64Bit)
        {
            string W_TMP = "XMM" ~ to!string(regno+8);
        }
    ...
    /*
        ...
        W[i:i+3] += {K,K,K,K}
    */
    private string[] precalc_16_31(int i)
    {
        int regno = regno(i);

        string W = "XMM" ~ to!string(regno);
        string W_minus_4 = "XMM" ~ to!string((regno-1)&7);
        string W_minus_8 = "XMM" ~ to!string((regno-2)&7);
        string W_minus_12 = "XMM" ~ to!string((regno-3)&7);
        string W_minus_16 = "XMM" ~ to!string((regno-4)&7);
        version(_32Bit)
        {
            string W_TMP = "XMM" ~ to!string((regno+1)&7);
            string W_TMP2 = "XMM" ~ to!string((regno+2)&7);
        }
    ...
    private string[] precalc_32_79(int i)
    {
        int regno = regno(i);
        ...
        version(_32Bit)
        {
            string W_minus_28 = "[ESP + WI_PTR + "~ to!string((regno-7)&7)~"*16]";
            string W_minus_32 = "[ESP + WI_PTR + "~ to!string((regno-8)&7)~"*16]";
            string W_TMP = "XMM" ~ to!string((regno+1)&7);
            string W_TMP2 = "XMM" ~ to!string((regno+2)&7);
        }
        version(_64Bit)
        {
            string W_minus_28 = "XMM" ~ to!string((regno-7)&7);
            string W_minus_32 = "XMM" ~ to!string((regno-8)&7);
        }
     ...
     */
    private string[] round(int i, string a, string b, string c, string d, string e)
    {
    /+    // Returns result in T1; may destroy T2
        return F(i, b, c, d) ~
               precalc(PRECALC_AHEAD + i) ~
               ["add "~e~","~WiKi(i),
                "ror "~b~",2",
                "mov "~T2~","~a,
                "add "~d~","~WiKi(i+1),
     ...
     *
     */
    public void transformSSSE3(uint* /*[5]*/ state, ubyte* /*[64]*/ buffer)
    {
...
debug(shaasm)
{
    ...
    void main()
    {
        writefln("%1$s", wrap(precalc_00_15()));
    }
}
