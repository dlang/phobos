...
template ThompsonOps(E, S, bool withInput:true)
{
    ...
    static bool op(IR code:IR.Wordboundary)(E* e, S* s)
    {
        with(e) with(s)
        {
            dchar back;
    ...
    static bool op(IR code:IR.Notwordboundary)(E* e, S* s)
    {
        with(e) with(s)
        {
            dchar back;
    ...
    static bool op(IR code:IR.Bol)(E* e, S* s)
    {
        with(e) with(s)
        {
            dchar back;
    ...
    static bool op(IR code:IR.Eol)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.InfiniteStart)(E* e, S* s)
    {
        with(e) with(s)
            t.pc += re.ir[t.pc].data + IRL!(IR.InfiniteStart);
        return op!(IR.InfiniteEnd)(e,s);
    }

    static bool op(IR code:IR.InfiniteQStart)(E* e, S* s)
    {
        with(e) with(s)
            t.pc += re.ir[t.pc].data + IRL!(IR.InfiniteQStart);
        return op!(IR.InfiniteQEnd)(e,s);
    }

    static bool op(IR code:IR.RepeatStart)(E* e, S* s)
    {
        with(e) with(s)
            t.pc += re.ir[t.pc].data + IRL!(IR.RepeatStart);
        return op!(IR.RepeatEnd)(e,s);
    }

    static bool op(IR code:IR.RepeatQStart)(E* e, S* s)
    {
        with(e) with(s)
            t.pc += re.ir[t.pc].data + IRL!(IR.RepeatQStart);
        return op!(IR.RepeatQEnd)(e,s);
    }

    static bool op(IR code)(E* e, S* s)
    ...
    static bool op(IR code:IR.OrEnd)(E* e, S* s)
    {
        with(e) with(s)
        {
            if(merge[re.ir[t.pc + 1].raw+t.counter] < genCounter)
    ...
    static bool op(IR code:IR.OrStart)(E* e, S* s)
    {
        with(e) with(s)
        {
            t.pc += IRL!(IR.OrStart);
            return op!(IR.Option)(e,s);
        }
    }

    static bool op(IR code:IR.Option)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.GotoEndOr)(E* e, S* s)
    {
        with(e) with(s)
        {
            t.pc = t.pc + re.ir[t.pc].data + IRL!(IR.GotoEndOr);
            return op!(IR.OrEnd)(e, s);
        }
    }

    static bool op(IR code:IR.GroupStart)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.GroupEnd)(E* e, S* s)
    {
        with(e) with(s)
        {
            uint n = re.ir[t.pc].data;
    ...
    static bool op(IR code)(E* e, S* s)
        if(code == IR.LookaheadEnd || IR.NeglookaheadEnd ||
    ...
    static bool op(IR code:IR.Nop)(E* e, S* s)
    {
        with(s) t.pc += IRL!(IR.Nop);
    }

    static bool op(IR code:IR.OrChar)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.Char)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.Any)(E* e, S* s)
    {
        with(e) with(s)
        {
            t.pc += IRL!(IR.Any);
    ...
    static bool op(IR code:IR.CodepointSet)(E* e, S* s)
    {
        with(e) with(s)
        {
    ...
    static bool op(IR code:IR.Trie)(E* e, S* s)
    {
        with(e) with(s)
        {
...
template ThompsonOps(E,S, bool withInput:false)
{
    ...
    {
        return ThompsonOps!(E,S,true).op!code(e,s);
    }
}
    ...
    if(is(Char : dchar))
{
    ...
    static struct State
    {
        ...
        bool popState(ThompsonMatcher* e)
        {
        	with(e)
        	{
        		recycle(t);
    ...
    static struct Ops(bool withInput) {
	mixin ThompsonOps!(ThompsonMatcher, State, withInput);
    }
    ...
    L_Eval:
        for(;;)
        {
            ...
            switch(re.ir[state.t.pc].code)
            {
                foreach(e; __traits(allMembers, IR))
                {
            mixin(`case IR.`~e~`: 
                if(!Ops!withInput.op!(IR.`~e~`)(&this, state))
                    break L_Eval;
                break;
                `);
                }
            default:
                assert(0, "Unrecognized instruction "~re.ir[state.t.pc].mnemonic);
            }
        }
