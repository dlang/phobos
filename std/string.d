...
--------------------
  +/
C[] translate(C = immutable char)(in char[] str, string transTable, in char[] toRemove = null) @trusted
    if(is(Unqual!C == char))
in
{
    assert(transTable.length == 128);
    foreach(c; chain(str, transTable, toRemove))
        assert(isASCII(c));
}
body
{
...
/++ Ditto +/
string makeTrans(in char[] from, in char[] to) @trusted
in
{
