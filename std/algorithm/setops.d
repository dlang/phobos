...
All of the the functions contained in this submodule, except
$(LREF cartesianProduct), expect sorted ranges as input, otherwise
the behaviour is independent of the specification.

The algorithms implemented are not limited at working only with sets
(collections of distinct objects), but also with multisets (collections of
objects which may contain duplicates). The rule of thumb is the following:
if the input contains only sets, the result is going to be a set, otherwise
if the input contains at least one multiset, the result is going to be a
multiset.
...
If at least one of the ranges is a multiset, then all of the
occurences of a duplicate element are taken into account. The result
is equivalent to merging all the ranges and picking the most
frequent $(D tgt.length) elements.
...
If at least one of the ranges is a multiset, then all of the occurences
of a duplicate element are taken into account. The result
is equivalent to merging all the ranges and picking the highest $(D tgt.length)
...
The length of the resulting range is going to be in all cases
the sum of all the lengths of the ranges passed as input. This
means that all elements (duplicates or not) are transferred to
the resulting range.
...
times in $(D r1) and `y` times and $(D r2), the number of occurentes
of `a` in the result range is going to be `x-y` if x > y or 0 othwerise.
...
If all the ranges are sets (do not contain duplicated elements),
...
the minimum number of occurences of a given element, propagates the
number of occurences of that element in the resulting range.
...
If both ranges are sets (do not contain duplicate elements), the resulting
