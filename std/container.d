...
$(D DList)s use reference semantics. They will lazilly auto initialize on the
first modifying operation. Lists that are initialized will alias to each other
on assignement.

Operations on $(D DList) are mostly stable. The only operations that
are unstable are the $(D remove) flavored functions. Any range overlapping with
removed nodes will be instantly invalidated.
 */
struct DList(T)
{
    //The generic Node, payload agnostic
    //This node does not carry a payload, and is used as the "sentinel" nodes
    //TODO: This struct does not depend on T, and should be moved outside of DList,
    //Along with other algorithms that do not depend on T.
    //This is why it was pre-emptivelly named DLNode.
    private static struct DLNode
    {
        DLNode* _prev;
        DLNode* _next;

        // Important note:
        // Do NOT place a constructor inside DLNode, in particular,
        // Do NOT auto link _prev and _next here.
        // This is because DLNode is nested inside PayNode.
        // While DLPaynodes are allocated on the heap and never moved,
        // DLNodes are moved into DLNodes when being constructed.
    }

    // Ditto, but with a payload.
    // The actual payloads. Uses "C"-style inheritance to DLNode (first arg is DLNode),
    // Allowing pointer casts back and forth.
    // The point of this "two-level" node system is that most operations (insert, remove)
    // Don't actually need to access the payload. They operate on the basic node, be it the
    // "Sentinel" node or a "Payload" node.
    private static struct PayNode
    {
        DLNode _n;
        static assert (PayNode._n.offsetof == 0);
    ...
    //Convenience: extract a payload out of a DLNode.
    private static @property ref inout(T) _payload(inout(DLNode*) pNode)
    {
        return (cast(inout(PayNode)*)pNode)._payload;
    }

    /*
    Implementation note:
    _head is a sentinel that is past the ends of the actual chain of nodes. This is
    the classic circular list implementation, which is trivial to implement, yet efficient.

    Note that when empty, _head will contain two internal pointers. This is theoretically
    illegal in D, due to move semantics. Because DLNode is heap allocated though, this is
    fine.
    */
    ...
    private @property bool _initialized() const
    {
        return cast(bool) _head;
    }

/**
Initialization. A $(D DList) can be intialized with an
indiscriminate amount of arguments, or an input range.
     */
    this(U)(U[] values...) if (isImplicitlyConvertible!(U, T))
    {
        insert(values);
...
/**
Comparison for equality. This will return $(D true) if both lists contain the
same sequence.
...
elements in $(D rhs).
    ...
    bool opEquals(ref const DList rhs) const
    {
        ...
        if (!lhs._initialized) return rhs.empty;
        if (!rhs._initialized) return lhs.empty;
        if (lhs._head == rhs._head) return true;

        const(DLNode)* nLhs = lhs._head._next;
        const(DLNode)* nRhs = rhs._head._next;
        while(true)
        {
            if (nLhs == _head) return nRhs == rhs._head;
            if (_payload(nLhs) != _payload(nRhs)) return false;
            nLhs = nLhs._next;
            nRhs = nRhs._next;
        }
    }
    ...
    struct Range
    {
        private DLNode* _first;
        private DLNode* _last;
        private this(DLNode* first, DLNode* last)
        {
            assert(!!_first == !!_last, "Dlist.Rangethis: Invalid arguments");
            _first = first; _last = last;
        }
        private this(DLNode* n) { _first = _last = n; }
...
Complexity: $(BIGOH 1)
    ...
    bool empty()
    {
        return _head is null || (_head._next == _head);
    }
...
Complexity: $(BIGOH 1)
    ...
    Range opSlice()
    {
        if (empty)
            return Range(null);
        return Range(_head._next, _head._prev);
    }
...
Complexity: $(BIGOH 1)
    ...
    static if(isAssignable!(T,T))
    {
        ...
        {
            assert(!empty, "DList.front: List is empty");
            move(value, _payload(_head._next));
        }
    }
...
Complexity: $(BIGOH 1)
    ...
    static if(isAssignable!(T,T))
    {
        ...
        {
            assert(!empty, "DList.back: List is empty");
            move(value, _payload(_head._prev));
        }
    }
...
/**
Removes all contents from the $(D DList). The resulting list will be empty, but
still initialized
...
Complexity: $(BIGOH log(n))
    ...
    {
        _ensureInitialized();
        return insertBeforeNode(_head._next, stuff);
    }
    ...
    {
        _ensureInitialized();
        return insertBeforeNode(_head, stuff);
    }
...
Complexity: $(BIGOH 1).
    ...
    T removeAny()
    {
        ...
        auto result = move(_payload(_head._prev));
        removeBack();
        return result;
    }
...
Complexity: $(BIGOH 1).
    ...
    //TODO: Move out of DList
    private void removeNode(DLNode* p)
    {
        p._prev._next = p._next;
        p._next._prev = p._prev;

        //Excise the removed range
        //We set the removed nodes pointers to null. This helps catch bugs.
        //It's not strictly necessary, and has a cost, so we only do this
        //in an assert block.
        version(assert)
        {
            p._next = null;
...
do not throw if they could not remove $(D howMany) elements. Instead,
if $(D howMany > n), all elements are removed. The returned value is
the effective number of elements removed.
...
Complexity: $(BIGOH howMany * log(n)).
    ...
    {
        size_t result;
        if (!empty && result < howMany)
        {
            auto p = _head;
            do
            {
                p = p._next;
                version(assert) p._prev._next = null;
                version(assert) p._prev = null;
                ++result;
            } while (p._next != _head && result < howMany);

            _head._next = p._next;
            _head._next._prev = _head;
            version(assert) p._next = null;
        }
        return result;
    }
    /// ditto
    ...
    size_t removeBack(size_t howMany)
    {
        ...
        if (!empty && result < howMany)
        {
            ...
            {
                p = p._prev;
                version(assert) p._next._prev = null;
                version(assert) p._next = null;
                ++result;
            } while (p._prev != _head && result < howMany);
...
$(D r) and $(D m) is the length of $(D stuff).
    ...
    size_t insertBefore(Stuff)(Range r, Stuff stuff)
    {
        enforce(!r.empty, "Cannot insertBefore an empty range");
        assert(_initialized, "Logic error: This range does not belong to this");
        DLNode* n = r._first;
    ...
    size_t insertAfter(Stuff)(Range r, Stuff stuff)
    {
        ...
        assert(_initialized, "Logic error: This range does not belong to this");
        DLNode* n = r._last;
        return insertBeforeNode(n._next, stuff);
    }

    /// ditto
    alias stableInsertAfter = insertAfter;

    // Helper: insert $(D stuff) before DLNode $(D n).
    private size_t insertBeforeNode(Stuff)(DLNode* n, Stuff stuff)
    if (isInputRange!Stuff && isImplicitlyConvertible!(ElementType!Stuff, T))
    {
        size_t result;
        if(stuff.empty) return result;
        DLNode* before = n._prev; //The node originally before n

        //The goal here is to create full "chain" before modifying our list.
        //If something throws, it preserves its integrity...

        //Do the first node manually
        auto first = cast(DLNode*) new PayNode(stuff.front);
        auto last = first;
        ++result;
        stuff.popFront();
        //And chain the rest...
        foreach (item; stuff)
        {
            last = cast(DLNode*) new PayNode(item, last);
            ++result;
        }
        
        //Now insert the chain.
        //TODO: Move into helper function out of DList
        before._next = first;  // before -> first ...
        first._prev  = before; // before <- first ...
        last._next   = n;      //     ... last   -> n
        n._prev      = last;   //     ... last   <- n
    ...
    {
        //self inserting
        //This is exception safe. (in the sense that an exception won't corrupt)
        new PayNode(stuff, n._prev, n);
        return 1;
    }
...
Ranges that overlap with $(D r) will be invalidated.
...
Complexity: $(BIGOH 1)
    ...
    Range remove(Range r)
    {
        ...
        //Naming the variables for human beings...
        //Compiler should elide them.
        auto rFirst = r._first; //range first
        auto rLast  = r._last;  //range last
        auto bRFirst = rFirst._prev; //before range first
        auto aRLast  = rLast._next;  //after  range last

        //Excise the removed range
        //We null terminate the removed range in assert to catch any
        //wrong future useage of removed nodes.
        bRFirst._next = aRLast;  // bRFirst -> aRLast
        aRLast._prev  = bRFirst; // bRFirst <- aRLast
        version(assert) rFirst._prev = null;    // null <- rFirst ...
        version(assert) rLast._next  = null;    //    ... rLast -> null
        return r;
    }
...
unittest
{
    ...
    for( ; !range.empty; range.popFront())
    {
        ...
        if (item == 2)
        {
            //This invalidates range
            list.linearRemove(take(range,1));
            break;
        }
    }
    ...
    for( ; !range.empty; range.popFront())
    {
        ...
        if (item == 2)
        {
            //This does not invalidate range
            list.linearRemove(take(range,2));
        }
    }
...
unittest
{
    ...
    //assert(r.equal([1, 2, 3, 7, 4])); //r is invalidated
}
