...
class Thread
{
    ...
    static void pauseAll()
    {
        synchronized (Thread.classinfo)
        {
            if (nthreads > 1)
            {
                thread_id thisid = CurrentId;
                int npause = 0;

                for (int i = 0; i < allThreadsDim; i++)
                {
                    Thread t = allThreads[i];
                    if (t && !IsEqualId(thisid, t.id) && t.state == TS.RUNNING)
                    {
						version (Win32)
						{
	                        t.pause();
    ...
    static void resumeAll()
    {
        synchronized (Thread.classinfo)
        {
            if (nthreads > 1)
            {
                ...
                {
                    Thread t = allThreads[i];
                    if (t && !IsEqualId(t.id, thisid) && t.state == TS.RUNNING)
                        t.resume();
                }
            }
        }
    ...
    static void yield()
    {
        version (Win32) Sleep(0);
		version (linux) sched_yield();
    }
  ...
  private:
	...
	static bool IsEqualId(thread_id id1, thread_id id2)
	{
		version (Win32) return id1 == id2;
		version (linux) return pthread_equal(id1, id2) != 0;
	}
	...
	bool create()
	{
		version (Win32)
		{
        	...
        	return hdl != cast(thread_hdl)0;
		} // Win32
		version (linux)
		{
        	// Create with thread attributes to allow non-default stack size - Dave Fladebo
        	return pthread_create(&id, &threadAttrs, &threadstart, cast(void*)this) == 0;
		} // linux
	}
    ...
    void waitForDeath()
    {
		version (Win32)
		{
        	DWORD dw = WaitForSingleObject(hdl, 0xFFFFFFFF);
        	state = TS.FINISHED;
        	CloseHandle(hdl);
        	hdl = null;
		} // Win32
	...
	version (linux)
	{
    	extern (C) static void *threadstart(void *p)
    	{
        	...
        	try
        	{
				...
				{
					// signal the wait condition (see the timed wait function)
