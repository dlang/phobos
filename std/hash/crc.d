 ...
 *
 * Note:
 * CRCs are usually printed with the MSB first. When using $(XREF hash.hash, toHexString) the result
 * will be in an unexpected order. Use $(XREF hash.hash, toHexString)s optional order parameter
 ...
 *
 * Source: $(PHOBOSSRC std/hash/_crc.d)
 *
 * Macros:
 ...
 * import std.hash.crc;
 *
 * auto hash = crc32Of("The quick brown fox jumps over the lazy dog");
 ...
 * auto hash = crc.digest("The quick brown fox jumps over the lazy dog");
 * assert(crcHexString(hash) == "414FA339");
 *
 * //Feeding data
 * ubyte[1024] data;
...
struct CRC32
{
    ...
    public:
        ...
        /**
         * Returns the finished CRC32 digest. This also calls $(LREF start).
         *
         * Note:
         * Providing a buffer doesn't make much sense here, as 4 ubytes can be returned in one
         * register.
         *
         * Examples:
         * --------
        ...
        ///ditto
        @trusted pure nothrow void finish(ref ubyte[4] data)
        {
            peek(data);
            start();
        }
        ...
        ///ditto
        @trusted pure nothrow void peek(ref ubyte[4] data) const
        {
            data = peek();
        }
}
...
unittest
{
    ...
    crc.put(cast(ubyte[])"abcdefghijklmnopqrstuvwxyz");
    crc.finish(digest);
    assert(digest == cast(ubyte[])x"bd50274c");
 ...
 * ---------
 */
public alias digest!CRC32 crc32Of;

/**
 * This is a convenience alias for $(XREF hash.hash, toHexString) producing the usual
 * CRC32 string output.
 */
 ...
 * See $(D std.hash.hash) for differences between template and OOP API.
 * 
 * This is an alias for $(XREF hash.hash, WrapperDigest)!CRC32, see
