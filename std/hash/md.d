 ...
 * Computes MD5 digests of arbitrary data. MD5 digests are 16 byte quantities that are like a
 * checksum or CRC, but are more robust.
 *
 * This module conforms to the APIs defined in $(D std.hash.hash). To understand the
 * differences between the template and the OOP API, see $(D std.hash.hash).
 *
 * This module publicly imports $(D std.hash.hash) and can be used as a stand-alone
 * module.
 *
 ...
 *
 * Source: $(PHOBOSSRC std/hash/_md.d)
 *
 * Macros:
 * WIKI = Phobos/StdMd5
 ...
 * See $(D std.hash.hash) for differences between template and OOP API.
 *
 * Examples:
 * --------
...
struct MD5
{
    ...
    public:
        ...
        /**
         * Returns the finished MD5 digest. This also calls $(LREF start).
         *
         * Examples:
         * --------
         * //Simple example
        ...
        ///ditto
        @trusted nothrow pure void finish(ref ubyte[16] data)
        {
            ubyte[8] bits = void;
            uint index, padLen;
...
/**
 * This is a convenience alias for $(XREF hash.hash, digest) using the
 * MD5 implementation.
 *
 * Examples:
 * ---------
 ...
 * See $(D std.hash.hash) for differences between template and OOP API.
 *
 * This is an alias for $(XREF hash.hash, WrapperDigest)!MD5, see
 * $(XREF hash.hash, WrapperDigest) for more information.
 *
 * Examples:
 * --------
